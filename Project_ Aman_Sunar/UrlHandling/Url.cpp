	...
	, _pathExtractor(nullptr)
{
	...
	if (!_locationExtractor->HasComponent() && _schemeExtractor->HasComponent()) {
		throw ::UrlFormatException("Error : Url has a scheme but is missing a location");
	}
...
{
	cout << url._schemeExtractor->GetComponent() << "://" + url._locationExtractor->GetComponent() <<  "/"  << url._pathExtractor->GetComponent() <<  "?" <<  url._queryExtractor->GetComponent()<<  "#"<< url._fragmentExtractor->GetComponent() << endl;
	return cout;
}
